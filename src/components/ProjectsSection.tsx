        ...
        <div className="container mx-auto px-6">
            ...
            <div className="flex gap-2 p-1 bg-white/10 backdrop-blur-md rounded-xl border border-white/20">
              {categories.map((category) => (
                <motion.button
                  ...
                  className={`px-6 py-2 rounded-lg transition-all duration-200 cursor-pointer font-medium ${
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredProjects.map((project, index) => (
              ...
              >
                <div className="bg-white/10 backdrop-blur-md rounded-xl border border-white/20 shadow-lg hover:shadow-xl hover:bg-white/15 transition-all duration-300 overflow-hidden">
                  ...
                  <div className="p-6">
                    ...
                    <h3 className="text-lg mb-2 group-hover:text-yellow-300 transition-colors duration-200 font-medium">
                      {project.title}
                    </h3>
                    <p className="text-white/70 text-sm line-clamp-2 font-normal">
                      {project.description}
                    </p>
                    <div className="flex flex-wrap gap-1 mt-4">
                      {project.tools.slice(0, 3).map((tool) => (
                        <span
                          key={tool}
                          className="px-2 py-1 bg-white/20 text-white backdrop-blur-sm rounded text-xs font-normal"
                        >
                          {tool}
                        </span>
                      ))}
                      {project.tools.length > 3 && (
                        <span className="px-2 py-1 text-white/50 text-xs font-normal">
                          +{project.tools.length - 3}
                        </span>
                      )}
                    </div>
